...
Open `notebooks/02-intro-to-r.qmd` in your Codespace to follow along -- it has
an empty code chunk under each exercise heading for your answers.
...
1. You have 83 chocolates in a bag. You would like to divide them into smaller
   bags of 8.
   i. How many small bags will you need?
   ii. After the bags are filled, how many extra chocolates will you have remaining?
2. You are planning a research study with the following eligibility criteria:
   ...
   Assuming you get all of this as the variables above, write an R command that
...
3. I have the following observations from a clinical parameter:
   ...
   However, I would prefer to limit the values to 1 decimal point. How would you
   do that?
4. Look up the `rnorm()` function in the Help Viewer. What arguments does this
   function take? Any default values?
5. Try to nest one function inside another.
...
# Data frame subsetting with square brackets: df[rows, columns]

# extract one column (by name) -- returns a vector
df$name

# extract row 2 only (leave the column slot empty to keep all columns)
df[2, ]

# extract rows 1 and 3
df[c(1, 3), ]

# extract column 3 only (leave the row slot empty to keep all rows)
df[, 3]

# extract rows based on one condition
df[df$year_of_birth > 1995, ]

# extract rows based on multiple conditions
df[df$year_of_birth > 1995 & df$favourite_colour == "red", ]
df[df$year_of_birth > 1995 | df$favourite_colour == "red", ]
...
Pick one exercise above that confused you. Ask Pi to explain its solution, then push back: *"why did you use X instead of Y?"*

Notice when Pi sounds confident vs. guessing. This habit -- using Pi as a study partner rather than an answer-machine -- is what will keep you learning all week.