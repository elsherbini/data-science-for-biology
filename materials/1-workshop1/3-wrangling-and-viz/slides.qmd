...
The AI writes better code when it knows the shape of your data: how many rows,
and what one row stands for (a participant, a participant-visit, a
smoker-category).

Not sure what a row means? Ask the AI what *it* thinks each row represents,
then check that answer against the table.
...
**Back of the room — agentic.** Work the exercise by chatting: ask for what
you want and let the assistant write and run the code for you.

**Front of the room — manual.** Type the code yourself. Reach for the
assistant only when you're stuck.
...
table_02 |>
  head(8) |>
  ggplot(aes(ph, nugent_score)) +
  geom_point(size = 4, color = "navyblue") +
  labs(x = "ph (one column)", y = "nugent_score (another column)") +
...
#| fig-width: 3
#| fig-height: 2.6
ggplot(table_02, aes(ph, nugent_score)) +
  geom_point(color = "navyblue") +
  theme_minimal(base_size = 11) +
  labs(title = "scatter", x = NULL, y = NULL)
```
...
## Every plot has three pieces {auto-animate=true}
...
#| fig-width: 5
#| fig-height: 4.6
ggplot(table_02, aes(ph, nugent_score, color = arm)) +
...
#| fig-width: 4.2
#| fig-height: 5
p_y <- ggplot(table_02, aes(arm, ph)) +
  geom_jitter(width = 0.2, color = "navyblue") +
  labs(title = "ph mapped to y") +
  theme_minimal(base_size = 13)

p_color <- ggplot(table_02, aes(arm, nugent_score, color = ph)) +
  geom_jitter(width = 0.2) +
  labs(title = "ph mapped to color") +
  theme_minimal(base_size = 13)

cowplot::plot_grid(p_y, p_color, ncol = 1)
```
:::
...
#| fig-width: 4.2
#| fig-height: 5
p_cont <- ggplot(table_02, aes(ph, crp_blood)) +
  geom_point(color = "navyblue") +
  labs(title = "continuous → y and x") +
  theme_minimal(base_size = 13)

p_cat <- ggplot(table_02, aes(arm, ph, fill = arm)) +
  geom_boxplot() +
  labs(title = "categorical → x and fill") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "none")

cowplot::plot_grid(p_cont, p_cat, ncol = 1)
```
:::
...
![Figure from [Claus O. Wilke, *Fundamentals of Data Visualization*](https://clauswilke.com/dataviz) (O'Reilly, 2019).](images/common-aesthetics-1.png)
...
## Exploring data for patterns {.your-turn}
...
Sketch at [excalidraw.com](https://excalidraw.com). Name your marks, your mappings, and your geom, then post the image and your answers to **#module-3-sketches** on Discord.
...
## Mapping vs parameter {auto-animate=true}
