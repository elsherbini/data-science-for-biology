...
- Transcript counts
    - Each transcript has a small chance of binding to cDNA, but there are many transcripts
    - The number of transcripts of a gene might depend on drug treatment
- Mutation count along a genome segment
    - Each base pair has a small chance of being mutated, but there are many base pairs overall
    - The number of mutations depends on selection pressure
- Infection count within a county
    - Each person has a small chance of being infected, but the population is large
...
All models can be used with covariates that are categorical. They are not
limited to drawing smooth curves or straight lines.
...
This is a linear regression model fitted to the previous slide's data, where
antibody levels are grouped.
