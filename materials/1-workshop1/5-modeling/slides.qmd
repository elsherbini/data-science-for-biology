...
Like having a well-stocked pantry is needed to cook many recipes, knowing the
basic statistical ingredients will allow you to adapt to whatever data or
...
**Data Story**: More social contact → higher probability of infection.
  ...
  - _Response $y$_: Who does (1) or doesn't (0) get infected during an outbreak?

  - _Covariates $x$_: Some people have larger social networks than others.
...
ggplot(infections) +
    ...
    geom_line(aes(social_contact, prob_infection), linewidth = 1, col = "#4ab05b") +
...
ggplot(infections) +
    geom_jitter(aes(social_contact, infected, col = factor(infected)), height = 0.05, size = 2) +
    geom_line(aes(social_contact, prob_infection), linewidth = 1, col = "#4ab05b") +
    geom_line(aes(social_contact, predicted_prob), linewidth = 1, col = "#e3a444") +
    scale_x_continuous(expand = c(0, 0)) +
    theme(legend.position = "bottom") +
    labs(y = "Infection Probability", x = "Level of Social Contact", col = "Infection Status")
```
...
ggplot(outbreak, aes(epicenter_distance)) +
    geom_line(aes(y = mean), col = "#4ab05b", linewidth = 1) +
...
ggplot(outbreak, aes(epicenter_distance)) +
    geom_point(aes(y = infected), size = 2) +
    geom_line(aes(y = mean), col = "#4ab05b", linewidth = 1) +
    geom_line(aes(y = predicted_mean), col = "#e3a444", linewidth = 1) +
    labs(x = "Epicenter Distance", y = "Infection Count")
```
