...
Yesterday: you built data stories. You picked a likelihood, fit a model, simulated from it, and revised until it matched what you saw.

Today: you write up what you found. A clinical paper has a Table 1 (who was in the study), a Table 2 (what differed between groups), and figures with p-values for the headline comparisons. Different audience, different tools — same statistical thinking underneath.

The right tool still falls out of the question. The question now is "how do I present this to a reader?"

## Goals for this module

By the end of this module you can:

- report the findings of a fitted model (including one of Kris's) in a publication-ready table
...
With six tests at α = 0.05, expecting one false positive by chance is the baseline, not the exception. Two standard fixes follow.
...
Both live in base R: `p.adjust(p_values, method = "bonferroni")` and `p.adjust(p_values, method = "BH")`.
...
Which cytokines survive correction is the result worth reporting. Bonferroni answers "any false positives?"; BH answers "what fraction of my calls are wrong?". For a 6-cytokine screen, BH is usually what you want.
...
# Reporting a model {.center}

## Kris's IL-1a model, refit
...
Treatment effect on log(IL-1a) varies by time point, accounting for within-participant correlation. The model exists. Now: how do we put it in a paper?

## Figure 1: the result, visible
...
The figure tells the same story the model does: the gap between arms is largest at week 1 and narrows by week 7.
...
`tbl_regression()` works on `lmer()` fits the same way it works on `lm()`. The output is the same shape — coefficient, estimate, CI, p-value — and the reader sees the treatment × time-point interaction estimates Kris's model produced.

The model and the report are different jobs. You build a model once (Module 5) and report it many times (Module 6) — never refit just to make a table.
