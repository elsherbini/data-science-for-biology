...
<div class="logo-preload" aria-hidden="true">
  ...
  <img id="logo-broom-mixed" src="assets/package-logos/broom_mixed_unofficial_substitute.svg" alt="">
</div>
  ...
  <div class="sheet-shell">
    <section class="sheet-hero" aria-labelledby="sheet-title">
      <div class="hero-copy">
        ...
        <div class="logo-disclaimer" role="note" aria-label="Unofficial logo note">
          <span class="logo-disclaimer-icon" aria-hidden="true">
            <svg viewBox="0 0 24 24" fill="none">
              <path d="M12 8v5m0 4h.01M10.3 4.2 2.8 17.1A2 2 0 0 0 4.5 20h15a2 2 0 0 0 1.7-2.9L13.7 4.2a2 2 0 0 0-3.4 0Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </span>
          <span><strong>Logo note:</strong> Some badges use unofficial substitute stickers where no verified official hex sticker was found. These are teaching visuals only and are not endorsed by the package authors.</span>
        </div>
      </div>
    </section>
...
<script>
  const packages = [
    ...
    },
    {
      ...
      modules: ["Module 6"],
      role: "Provides tidiers for mixed-model objects so table tools can understand them.",
      use: "Use it indirectly when `gtsummary` needs to turn `lmer()` output into a table.",
      functions: [
        ["tidy methods", "Used indirectly for mixed-model table output."],
        ["glance methods", "Used indirectly for model summaries when needed."]
      ],
      example: "fit |> tbl_regression()",
      watch: "The course loads `broom.mixed` but usually does not call its functions directly."
    }
  ];
  ...
  const packageMeta = {
    ...
    "broom.mixed": {
      priority: 18,
      logoId: "logo-broom-mixed",
      logoSource: "Unofficial substitute sticker bundle: original teaching graphic based on the CRAN package description.",
      logoStatus: "unofficial",
      task: "Tidy model output",
      learningGroup: "Tables and reporting",
      audience: "learning"
    },
    knitr: {
  ...
  const moduleMap = [
    ...
    ["Module 6", ["broom.mixed", "dplyr", "forcats", "ggplot2", "ggpubr", "gtsummary", "here", "lme4", "lmerTest", "patchwork", "readr", "tibble", "tidyverse"]]
  ];
